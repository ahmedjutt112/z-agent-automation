 ...
 * state change.
 */
export function createTrayIcon(state = "idle") {
 ...
 */
export function buildTrayMenu(window, state, workflows = [], tasks = []) {
 ...
 */
export function setupTray(window) {
    if (trayInstance) {
        return trayInstance;
    }
 ...
 */
export function setTrayState(state, window = null) {
    currentState = state;
    if (!trayInstance)
        return;
 ...
 */
export async function refreshTrayMenu(window) {
    let workflows = [];
    let tasks = [];
    try {
