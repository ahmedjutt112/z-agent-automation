 ...
 */
import { BrowserWindow, Menu, Tray, nativeImage, MenuItemConstructorOptions } from "electron";
...
/** Tray state surface — also exported for renderer type alignment. */
 ...
 */
const STATE_COLORS: Record<TrayState, [number, number, number]> = {
 ...
 */
function renderStateBitmap(state: TrayState): Buffer {
 ...
 * state change.
 */
export function createTrayIcon(state: TrayState = "idle"): Electron.NativeImage {
  const rgba = renderStateBitmap(state);
  const png = encodePng(rgba, ICON_SIZE);
  const img = nativeImage.createFromBuffer(png, { width: ICON_SIZE, height: ICON_SIZE });
...
export function buildTrayMenu(
  ...
  const workflowSubmenu: MenuItemConstructorOptions[] =
    workflows.length === 0
  ...
  const taskSubmenu: MenuItemConstructorOptions[] =
    tasks.length === 0
      ? [{ label: "(none)", enabled: false }]
 ...
 */
export function setupTray(window: BrowserWindow | null): Tray {
  if (trayInstance) {
    return trayInstance;
  }
  const tray = new Tray(createTrayIcon(currentState));
 ...
 */
export function setTrayState(state: TrayState, window: BrowserWindow | null = null): void {
  currentState = state;
 ...
 */
export async function refreshTrayMenu(window: BrowserWindow | null): Promise<void> {
  ...
  try {
    ...
    if (resp.ok) {
      const data = (await resp.json()) as WorkflowSummary[];
...
async function emergencyStop(window: BrowserWindow | null): Promise<void> {
  try {
...
async function runWorkflowById(workflowId: string, window: BrowserWindow | null): Promise<void> {
  try {
    ...
    if (resp.ok) {
      const result = (await resp.json()) as { run_id?: string };
