...
export function getAccentColorHex(accent: AccentColor): string {
  ...
  try {
    ...
    return {
      theme: VALID_THEMES.includes(parsed.theme as Theme)
        ...
        : DEFAULT_THEME.theme,
      accent: VALID_ACCENTS.includes(parsed.accent as AccentColor)
      ...
      font_scale:
        typeof parsed.font_scale === "number" &&
...
export function setTheme(config: ThemeConfig): void {
  ...
  window.dispatchEvent(
    new CustomEvent<ThemeConfig>("theme-change", { detail: config }),
  );
}
...
export function applyTheme(config: ThemeConfig): void {
  ...
  const effective =
    config.theme === "system" ? getSystemTheme() : config.theme;
...
export const DEFAULT_THEME_CONFIG: ThemeConfig = { ...DEFAULT_THEME };
