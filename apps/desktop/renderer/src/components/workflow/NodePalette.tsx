...
export interface PaletteItem {
  /** Display label, e.g. "Open Browser". */
  label: string;
  /** Backend tool / action name, e.g. "browser.open". */
  toolName: string;
  /** Visual kind — controls which React Flow node renderer is used. */
  visualType: NodeVisualType;
  /** Icon for the palette row. */
  icon: LucideIcon;
  /** Default args seed (e.g. a placeholder selector). */
...
export function NodePalette(): JSX.Element {
  ...
  const onDragStart = (e: React.DragEvent<HTMLDivElement>, item: PaletteItem): void => {
  ...
  return (
    <aside className="w-60 shrink-0 border-r border-zinc-800 bg-zinc-950/70 overflow-y-auto h-full">
          ...
          return (
            <div key={cat.name} className="mb-1">
              <button
                ...
                className="flex w-full items-center justify-between px-2 py-1 text-[11px] uppercase tracking-wider text-zinc-400 hover:text-zinc-200"
              >
                <span>{cat.name}</span>
              ...
              {!isCollapsed && (
                <ul className="space-y-0.5 mt-0.5">
                  {cat.items.map((item) => {
                    ...
                    return (
                      <li key={`${cat.name}-${item.label}`}>
                        <div
                          draggable
                          onDragStart={(e) => onDragStart(e, item)}
                          className="flex items-center gap-2 px-2 py-1.5 rounded text-xs text-zinc-200 bg-zinc-900/60 hover:bg-zinc-800 hover:ring-1 hover:ring-zinc-700 cursor-grab active:cursor-grabbing"
                          title={`Drag to add ${item.label} (${item.toolName})`}
                        >
